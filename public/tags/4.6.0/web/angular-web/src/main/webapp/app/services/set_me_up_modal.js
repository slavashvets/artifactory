...
//            let url = new URL(data.baseUrl) //CAUSES PROBLEM ON IE, NOT REALY NEEDED...
    ...
    _initSetMeUpScope() {
        ...
        this.setMeUpScope.resolveSnippet = (resolveDockerReverseProxy = true) => {
            ...
            if (this.setMeUpScope.status.snippetResolved) return;
    ...
    _resolveDockerAolSnippets(repoKey) {
                    ...
                    snippetsParts.forEach((part) => {
                        if (snip[part]) {
                            ...
                            if (repoKey === null) { //inject the default
                                ...
                                if (part === 'before' && _.contains(snip[part],'artprod.company.com')) {
                                    let brIndex = snip[part].indexOf('<br/>');
                                    snip[part] = snip[part].substr(0,brIndex);
                                }
        ...
        if (repoKey) {
            if (this.cachedDockerAolHostname) {
                if (this.cachedDockerAolHostname !== '$$$pending$$$') {
                    let serverName = this.cachedDockerAolHostname + '-' + repoKey + '.jfrog.io';
                    loopRun(serverName);
                }
            }
            else {
                this.cachedDockerAolHostname = '$$$pending$$$';
                this.dockerStatusDao.get({repoKey: repoKey}).$promise.then((data) => {
                    this.cachedDockerAolHostname = data.hostname;
                    let serverName = data.hostname + '-' + repoKey + '.jfrog.io';
                    loopRun(serverName);
                });
            }
        }
