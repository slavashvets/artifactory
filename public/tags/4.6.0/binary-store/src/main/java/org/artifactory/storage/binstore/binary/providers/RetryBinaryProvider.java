...
public class RetryBinaryProvider extends BinaryProviderBase {
    ...
    public void initialize() {
        this.maxTrays = getIntParam("maxTrays");
        this.interval = getIntParam("interval");
    }
    ...
    @Override
    public BinaryInfo addStream(BinaryStream inputStream) throws IOException {
    ...
    public StorageInfo getStorageInfo() {
        return getStorageInfo(0);
    }

    public StorageInfo getStorageInfo(int trying) {
        try {
            return next().getStorageInfo();
        } catch (Exception e) {
            if (trying < maxTrays) {
                waitDelayTime();
                log.trace(
                        "Failed to get storage info from next binary provider, trying again for the  '{}'  time",
                        trying);
                return getStorageInfo(++trying);
            } else {
                log.error("Failed get storage info from next binary provider", e);
                throw e;
            }
        }
    ...
    public BinaryInfo addStream(BinaryStream inputStream, int trying) throws IOException {
        try {
