...
public class MavenMetadataCalculationInterceptor extends StorageInterceptorAdapter {
    ...
    public void afterDelete(VfsItem fsItem, MutableStatusHolder statusHolder, DeleteContext ctx) {
        if (ctx.isCalculateMavenMetadata() && isMavenSupportingRepo(getLocalRepo(fsItem))) {
            ...
            RepoPath folderForMetadataCalculation = ctx.getRepoPath().getParent();
            if (folderForMetadataCalculation != null && !folderForMetadataCalculation.isRoot()) {
                if (fsItem.isFile()) {
                    // calculate maven metadata on the artifactId node (grandparent of the file)
                    folderForMetadataCalculation = folderForMetadataCalculation.getParent();
                }
                mmService.calculateMavenMetadataAsync(folderForMetadataCalculation, true);
            }
        }
    }
