...
export class AdminRepositoryFormController {
    constructor($q, $scope, $stateParams, $state, $timeout, RepositoriesDao, PropertySetsDao, ArtifactoryGridFactory, ReverseProxiesDao,
            ArtifactoryModal, ArtifactoryFeatures, ArtifactoryNotifications, commonGridColumns,ArtifactoryModelSaver, GeneralConfigDao, DockerStatusDao) {
        this.$scope = $scope;
        this.$q = $q;
                ...
                this.repositoriesDao.save(this.repoInfo).$promise.then((result)=> {
                    ...
                    if (this.repoInfo.typeSpecific.repoType === "Docker" && this.features.isAol()) {
                        ...
                        this.DockerStatusDao.get({repoKey: dockerData.repoKey}).$promise.then((data) => {
                            ...
                            dockerData.dockerPath = data.hostname + '-' + dockerData.repoKey + '.jfrog.io/hello-world';
                            dockerData.deployToLocal = data.deployToLocal;

                            switch(dockerData.packageType) {
                                case 'local':
                                    dockerData.snippets = [
                                        {message: "Tag an image.", snippet: "docker tag hello-world " + dockerData.dockerPath},
                                        {message: "Then push it to your repository.", snippet: "docker push " + dockerData.dockerPath}
                                    ];
                                    break;
                                case 'remote':
                                    dockerData.snippets = [
                                        {message: "Pull an image from your repository.", snippet: "docker pull " + dockerData.dockerPath}
                                    ];
                                    break;
                                case 'virtual':
                                    if (dockerData.deployToLocal){
                                        dockerData.snippets = [
                                            {message: "Pull an image from your repository.", snippet: "docker pull " + dockerData.dockerPath},
                                            {message: "And to test deploy to virtual, tag an image.", snippet: "docker tag hello-world " + dockerData.dockerPath},
                                            {message: "Then push it.", snippet: "docker push " + dockerData.dockerPath}
                                        ];
                                    } else if (!dockerData.deployToLocal) {
                                        dockerData.snippets = [
                                            {message: "Pull an image from your repository.", snippet: "docker pull " + dockerData.dockerPath}
                                        ];
                                    }
    ...
    openRepoTypeModal() {
        ...
        this.$repoTypeScope.highlightCheck = (typeFilter,type) => {
            if (typeFilter) {
                ...
                if (string.indexOf(searchstring) > -1 && searchstring.length > 0) {
