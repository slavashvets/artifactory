...
public class VirtualRepoDownloadStrategy {
    ...
    private RepoResource processStandard(InternalRequestContext context, RepoPath repoPath, List<RealRepo> repositories,
        ...
        UnfoundRepoResource forbiddenOrRejected = null;
        ...
        for (RealRepo repo : repositories) {
            ...
            } else if (forbiddenOrRejected == null) {
                forbiddenOrRejected = checkIfForbiddenOrRejected(res, repoPath, repo.getKey());
            }
        }
        ...
        if (forbiddenOrRejected != null) {
            RepoRequests.logToContext("Returning an unfound (forbidden or rejected) resource");
            return forbiddenOrRejected;
        } else {
    ...
    private UnfoundRepoResource checkIfForbiddenOrRejected(RepoResource resource, RepoPath path, String storingRepoKey) {
        if (resource instanceof UnfoundRepoResource) {
            UnfoundRepoResource original = ((UnfoundRepoResource) resource);
            int statusCode = original.getStatusCode();
            if (statusCode == HttpStatus.SC_FORBIDDEN || statusCode == HttpStatus.SC_CONFLICT) {
                String err;
                UnfoundRepoResourceReason.Reason reason;
                if (statusCode == HttpStatus.SC_FORBIDDEN) {
                    err = "forbidden";
                    reason = UnfoundRepoResourceReason.Reason.UNDEFINED;
                } else {
                    err =  "rejected";
                    reason = UnfoundRepoResourceReason.Reason.REJECTED;
                }
                log.warn(String.format("%S is %s by %s: %s", resource.getRepoPath().getPath(), err, storingRepoKey,
                        original.getDetail()));
                RepoRequests.logToContext("Request is %s by %s: %s", err, storingRepoKey, original.getDetail());
                return new UnfoundRepoResource(path, reason, original.getDetail(), original.getStatusCode());
            }
        }
        return null;
    }
