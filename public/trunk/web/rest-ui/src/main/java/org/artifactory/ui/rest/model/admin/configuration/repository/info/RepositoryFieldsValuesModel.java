...
public class RepositoryFieldsValuesModel {
    ...
    public RepositoryFieldsValuesModel(CentralConfigDescriptor descriptor, RepositoryService repositoryService) {
        ...
        packageTypes = Lists.newArrayList(RepoType.values()).stream().filter(t -> (t != RepoType.CocoaPods)).
                map(RepoType::name).collect(Collectors.toList());
        ...
        availableLocalRepos = repositoryService.getLocalRepoDescriptors().stream().filter(t -> (t.getType() != RepoType.CocoaPods))
                .map(LocalRepoDescriptor::getKey).collect(Collectors.toList());
        availableRemoteRepos = repositoryService.getRemoteRepoDescriptors().stream().filter(t -> (t.getType() != RepoType.CocoaPods)).
                map(RemoteRepoDescriptor::getKey).collect(Collectors.toList());
        availableVirtualRepos = repositoryService.getVirtualRepoDescriptors().stream().filter(t -> (t.getType() != RepoType.CocoaPods)).
                map(VirtualRepoDescriptor::getKey).collect(Collectors.toList());
