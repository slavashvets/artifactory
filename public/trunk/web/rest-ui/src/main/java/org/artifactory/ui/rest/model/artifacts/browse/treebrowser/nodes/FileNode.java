...
public class FileNode extends BaseNode {
    ...
    private void setUploadToBintrayAction(ItemInfo itemInfo, List<IAction> actions) {
        boolean isDocker = localOrCachedRepoDescriptor(getRepoPath()).getType().equals(RepoType.Docker);
        if (!itemInfo.isFolder() && !isDocker  && BintrayRestHelper.isPushToBintrayAllowed()) {
    ...
    public void updateNodeData() {
        if (isLocal()) {
            ...
            if (properties != null) {
                externalUrl = properties.getFirst("externalUrl");
            }
        }
    }
