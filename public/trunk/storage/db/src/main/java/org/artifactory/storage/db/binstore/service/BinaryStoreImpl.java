...
public class BinaryStoreImpl implements InternalBinaryStore {
    ...
    private BinaryInfo safeGetBinaryInfo(BinaryStoreInputStream in) {
    ...
    public int incrementNoDeleteLock(String sha1) {
        ...
        if (previous == null) {
            ...
            count = previous.incrementAndGet();
        }
        debugPrintIncrementStack(sha1, count);
        return count;
    }
    ...
    public void decrementNoDeleteLock(String sha1) {
        ...
        if (usageCount != null) {
            ...
            debugPrintDecrementStack(sha1, count);
        }
    }
    ...
    /**
     * prints calling stack on ref. counter dec.
     *
     * @param sha1 item triggers counter decrease
     */
    private void debugPrintDecrementStack(String sha1, int count) {
        log.debug("Decreasing ref. counter for '{}', new count is '{}', call stack is: {}",
                sha1, count, getCurrentThreadStackTrace()
        );
    }

    /**
     * prints calling stack on ref. counter inc.
     *
     * @param sha1 item triggers counter decrease
     */
    private void debugPrintIncrementStack(String sha1, int count) {
        log.debug("Increasing ref. counter for '{}', new count is '{}', call stack is: {}",
                sha1, count, getCurrentThreadStackTrace()
        );
    }

    /**
     * Prints currentThread stack trace
     */
    private List<String> getCurrentThreadStackTrace() {
        final List<String> frames = Lists.newLinkedList();
        if(log.isDebugEnabled()) {
            try {
                Arrays.stream(Thread.currentThread().getStackTrace())
                        .forEach(item -> {
                            frames.add(item.toString() + System.lineSeparator());
                        }
                );
            } catch (SecurityException e) {
                log.debug("SecurityException has been thrown: {}, no stack trace available", e.getMessage());
            }
        }
        return frames;
    }
