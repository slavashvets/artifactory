...
import org.artifactory.storage.binstore.config.model.Config;
import org.artifactory.storage.binstore.config.visitor.ConfigExplorer;
import org.artifactory.storage.binstore.config.visitor.FillDefaultValuesVisitor;
import org.artifactory.storage.binstore.config.visitor.MergeProvidersWithChainsVisitor;
import org.artifactory.storage.binstore.config.visitor.MergeVisitor;
...
public class BinaryProviderConfigBuilder {
     ...
     */
    public static ChainMetaData buildByUserConfig(InputStream defaultConfigStream, InputStream userConfigStream, BinaryProviderConfig defaultValues) {
        try {
            log.debug("Loading the default config");
            // Load user and default configs
            Config defaultConfig = loadConfig(defaultConfigStream);
            ...
            explorer.visit(defaultConfig, defaultConfig, new MergeProvidersWithChainsVisitor());
            // Fill Default values
            explorer.visit(defaultConfig, defaultConfig, new FillDefaultValuesVisitor(defaultValues));
            // Find Selected chain
