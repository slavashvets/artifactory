...
public class RepositoryServiceImpl implements InternalRepositoryService, LockableUndeploy {
    ...
    public RepoResource saveResource(StoringRepo repo, SaveResourceContext saveContext)
        ...
        boolean enterInTx = LockingAdvice.isActualTransactionActive();
        ...
        // TX Leak from RTFACT-9067
        if (!enterInTx && LockingAdvice.isActualTransactionActive()) {
            RuntimeException exception = new IllegalStateException(
                    "Artifactory Storage Session adding binary did not close transaction for " + saveContext.getRepoResource());
